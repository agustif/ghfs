...
function createContext(
  ...
  return {
    provider: createProviderMock({
      ...
      fetchRepositoryRulesets: vi.fn(async () => []),
      fetchRuleSuites: vi.fn(async () => []),
      fetchOidcCustomization: vi.fn(async () => null),
      fetchCopilotSeats: vi.fn(async () => null),
      fetchAuditLog: vi.fn(async () => []),
      fetchSecretScanningSettings: vi.fn(async () => null),
      fetchSecurityConfiguration: vi.fn(async () => null),
    }),
    repoSlug: 'owner/repo',
    storageDirAbsolute,
    config,
