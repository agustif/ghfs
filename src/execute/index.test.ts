...
function createMockProvider(overrides: Partial<RepositoryProvider> = {}): RepositoryProvider {
  return {
    ...
    fetchBillingUsageSummary: vi.fn(async () => null),
    fetchPackages: vi.fn(async () => []),
    fetchCodespaces: vi.fn(async () => []),
    fetchSecretScanningAlerts: vi.fn(async () => []),
    fetchSecretScanningAlertLocations: vi.fn(async () => []),
    fetchDependabotAlerts: vi.fn(async () => []),
    fetchCodeScanningAlerts: vi.fn(async () => []),
