...
`
      ...
      type GitHubTemplateRepoWire = {
        is_template?: boolean
        template_repository?: { full_name?: string | null } | null
      }

      const EMPTY_TEMPLATE_INFO = new TemplateInfo({
        isTemplate: false,
        templateRepository: null,
      })

      function mapTemplateInfo(row: GitHubTemplateRepoWire): TemplateInfo {
        return new TemplateInfo({
          isTemplate: Boolean(row.is_template),
          templateRepository: row.template_repository?.full_name ?? null,
        })
      }

      const fetchTemplateInfo = Effect.fn("GitHubClient.fetchTemplateInfo")(
        function* (): Effect.fn.Return<TemplateInfo, GitHubError> {
          return yield* Effect.gen(function* () {
            const response = yield* client
            ...
            const json = (yield* response.json.pipe(
              Effect.mapError(toGitHubError)
            )) as GitHubTemplateRepoWire
            return mapTemplateInfo(json)
          }).pipe(
            Effect.catchAll(() => Effect.succeed(EMPTY_TEMPLATE_INFO))
          )
        }
      )
      ...
      const EMPTY_NETWORK_SUMMARY = new NetworkSummary({
        forks: 0,
        subscribers: 0,
        watchers: 0,
        networkCount: 0,
      ...
      function mapNetworkSummary(row: GitHubNetworkRepoWire): NetworkSummary {
        return new NetworkSummary({
          forks: row.forks_count ?? 0,
          subscribers: row.subscribers_count ?? 0,
          watchers: row.watchers_count ?? 0,
          networkCount: row.network_count ?? 0,
        })
      }

      const fetchNetworkSummary = Effect.fn("GitHubClient.fetchNetworkSummary")(
        function* (): Effect.fn.Return<NetworkSummary, GitHubError> {
          return yield* Effect.gen(function* () {
            const response = yield* client
              .get(`/repos/${owner}/${name}`)
              .pipe(Effect.mapError(toGitHubError))
            const json = (yield* response.json.pipe(
              Effect.mapError(toGitHubError)
            )) as GitHubNetworkRepoWire
            return mapNetworkSummary(json)
          }).pipe(
            Effect.catchAll(() => Effect.succeed(EMPTY_NETWORK_SUMMARY))
          )
        }
      )
      ...
      function mapIssueType(row: GitHubIssueTypeWire): IssueType {
      ...
      const fetchRepositoryIssueTypes = Effect.fn(
        "GitHubClient.fetchRepositoryIssueTypes"
      )(function* (): Effect.fn.Return<Array<IssueType>, GitHubError> {
        return yield* Effect.gen(function* () {
          const response = yield* client
            .get(`/repos/${owner}/${name}/issue-types`)
            .pipe(Effect.mapError(toGitHubError))
          const json = yield* response.json.pipe(Effect.mapError(toGitHubError))
          const rows = (Array.isArray(json) ? json : []) as Array<GitHubIssueTypeWire>
          ...
          Effect.catchIf(
            (error): error is GitHubError =>
              error instanceof GitHubError &&
              (error.status === 404 || error.status === 403),
            () => Effect.succeed([] as Array<IssueType>)
          )
        )
      })
...
`
      ...
      type GitHubCommitActivityWire = {
        days?: Array<number> | null
        total?: number | null
        week?: number | null
      }

      function mapCommitActivity(
        row: GitHubCommitActivityWire
      ): CommitActivity | null {
        if (
          !Array.isArray(row.days) ||
          !row.days.every((d) => typeof d === "number") ||
      ...
      const fetchCommitActivity = Effect.fn(
        "GitHubClient.fetchCommitActivity"
      )(function* (): Effect.fn.Return<Array<CommitActivity>, GitHubError> {
        return yield* Effect.gen(function* () {
          const response = yield* client
          ...
          const json = yield* response.json.pipe(Effect.mapError(toGitHubError))
          const rows = (Array.isArray(json) ? json : []) as Array<
            GitHubCommitActivityWire
          >
          return rows.flatMap((row) => {
            const mapped = mapCommitActivity(row)
            return mapped ? [mapped] : []
          })
        }).pipe(
          Effect.catchIf(
            (error): error is GitHubError =>
              ...
              (error.status === 404 ||
                error.status === 403 ||
                error.status === 202),
            () => Effect.succeed([] as Array<CommitActivity>)
          )
        )
      })
      ...
      function mapGitRef(row: GitHubGitRefWire): GitRef | null {
        const ref = row.ref?.trim()
        const nodeId = row.node_id?.trim()
        const url = row.url?.trim()
        const objectType = row.object?.type?.trim()
        const objectSha = row.object?.sha?.trim()
        const objectUrl = row.object?.url?.trim()
        if (!ref || !nodeId || !url || !objectType || !objectSha || !objectUrl) {
          return null
        }
        ...
        return new GitRef({
          ...
          nodeId,
          url,
          object: { type: objectType, sha: objectSha, url: objectUrl }
        })
      }

      function normalizeGitRefRows(json: unknown): Array<GitHubGitRefWire> {
        if (Array.isArray(json)) return json as Array<GitHubGitRefWire>
        if (json && typeof json === "object") return [json as GitHubGitRefWire]
        return []
      }

      const fetchGitRefs = Effect.fn("GitHubClient.fetchGitRefs")(function* (params: {
        page?: number
        perPage?: number
      } = {}): Effect.fn.Return<Array<GitRef>, GitHubError> {
        return yield* Effect.gen(function* () {
          const searchParams = new URLSearchParams()
          if (params.page) searchParams.set("page", String(params.page))
          searchParams.set("per_page", String(params.perPage ?? 100))
          ...
          const response = yield* client
            .get(`/repos/${owner}/${name}/git/refs?${searchParams.toString()}`)
            .pipe(Effect.mapError(toGitHubError))
          const json = yield* response.json.pipe(Effect.mapError(toGitHubError))
          const rows = normalizeGitRefRows(json)
          return rows.flatMap((row) => {
            const mapped = mapGitRef(row)
            return mapped ? [mapped] : []
          })
        }).pipe(
          Effect.catchIf(
            (error): error is GitHubError =>
              error instanceof GitHubError &&
              (error.status === 404 || error.status === 409 || error.status === 403),
            () => Effect.succeed([] as Array<GitRef>)
          )
        )
      })
      ...
      type GitHubTrafficReferrerWire = {
        referrer?: string | null
        count?: number | null
        uniques?: number | null
      }

      function mapTrafficReferrer(
        row: GitHubTrafficReferrerWire
      ...
      const fetchTrafficReferrers = Effect.fn(
        "GitHubClient.fetchTrafficReferrers"
      )(function* (): Effect.fn.Return<Array<TrafficReferrer>, GitHubError> {
        return yield* Effect.gen(function* () {
          const response = yield* client
          ...
          const json = yield* response.json.pipe(Effect.mapError(toGitHubError))
          const rows = (Array.isArray(json) ? json : []) as Array<
            GitHubTrafficReferrerWire
          >
          return rows.flatMap((row) => {
            const mapped = mapTrafficReferrer(row)
            return mapped ? [mapped] : []
          })
        }).pipe(
          Effect.catchIf(
            (error): error is GitHubError =>
              error instanceof GitHubError &&
              (error.status === 404 || error.status === 403),
            () => Effect.succeed([] as Array<TrafficReferrer>)
          )
        )
      })
      ...
      type GitHubTrafficViewDayWire = {
        timestamp?: string | null
        count?: number | null
        uniques?: number | null
      }

      type GitHubTrafficViewsWire = {
        count?: number | null
        uniques?: number | null
        views?: Array<GitHubTrafficViewDayWire> | null
      }

      function mapTrafficViewDay(row: GitHubTrafficViewDayWire) {
        ...
        return { timestamp, count: row.count, uniques: row.uniques }
      }

      function mapTrafficViews(row: GitHubTrafficViewsWire): TrafficViews {
        const views = Array.isArray(row.views)
          ? row.views.flatMap((day) => {
              const mapped = mapTrafficViewDay(day)
              return mapped ? [mapped] : []
            })
          : []
        return new TrafficViews({
          count: typeof row.count === "number" ? row.count : 0,
          uniques: typeof row.uniques === "number" ? row.uniques : 0,
          views
        })
      }

      const emptyTrafficViews = (): TrafficViews =>
        new TrafficViews({ count: 0, uniques: 0, views: [] })

      const fetchTrafficViews = Effect.fn("GitHubClient.fetchTrafficViews")(function* (): Effect.fn.Return<
        TrafficViews,
        GitHubError
      > {
        return yield* Effect.gen(function* () {
          const response = yield* client
            ...
            .pipe(Effect.mapError(toGitHubError))
          const json = yield* response.json.pipe(Effect.mapError(toGitHubError))
          const row = (json && typeof json === "object" ? json : {}) as GitHubTrafficViewsWire
          return mapTrafficViews(row)
        }).pipe(
          Effect.catchIf(
            (error): error is GitHubError =>
              error instanceof GitHubError &&
              (error.status === 404 || error.status === 403),
            () => Effect.succeed(emptyTrafficViews())
          )
        )
      })


      type GitHubTrafficCloneDayWire = {
        timestamp?: string | null
        count?: number | null
        uniques?: number | null
      }

      type GitHubTrafficClonesWire = {
        count?: number | null
        uniques?: number | null
        clones?: Array<GitHubTrafficCloneDayWire> | null
      }

      function mapTrafficCloneDay(row: GitHubTrafficCloneDayWire) {
        const timestamp = row.timestamp?.trim()
        if (
          !timestamp ||
          typeof row.count !== "number" ||
      ...
      function mapTrafficClones(row: GitHubTrafficClonesWire): TrafficClones {
        const clones = Array.isArray(row.clones)
          ? row.clones.flatMap((day) => {
              const mapped = mapTrafficCloneDay(day)
              return mapped ? [mapped] : []
            })
          : []
        return new TrafficClones({
          count: typeof row.count === "number" ? row.count : 0,
          uniques: typeof row.uniques === "number" ? row.uniques : 0,
          clones
        })
      ...
      const emptyTrafficClones = (): TrafficClones =>
        new TrafficClones({ count: 0, uniques: 0, clones: [] })

      const fetchTrafficClones = Effect.fn("GitHubClient.fetchTrafficClones")(function* (): Effect.fn.Return<
        TrafficClones,
        GitHubError
      > {
        return yield* Effect.gen(function* () {
          const response = yield* client
            ...
            .pipe(Effect.mapError(toGitHubError))
          const json = yield* response.json.pipe(Effect.mapError(toGitHubError))
          const row = (json && typeof json === "object" ? json : {}) as GitHubTrafficClonesWire
          return mapTrafficClones(row)
        }).pipe(
          Effect.catchIf(
            (error): error is GitHubError =>
              error instanceof GitHubError &&
              (error.status === 404 || error.status === 403),
            () => Effect.succeed(emptyTrafficClones())
          )
        )
      })
